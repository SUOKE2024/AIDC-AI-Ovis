    ...
    try {
      ...
      if (permissionGranted) {
        ...
        setState(() {
          _messages.add({
            'sender': 'ai',
            'content': '看起来没有获得麦克风权限。您可以在设备设置 > 隐私与安全 > 麦克风 中找到本应用并手动授予权限，然后重新点击麦克风图标。',
            'timestamp': DateTime.now(),
            'isRead': true,
          });
        });
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('权限请求出错: ${e.toString().substring(0, math.min(50, e.toString().length))}...'),
            duration: const Duration(seconds: 3),
          ),
        );
      }
...
''';
  ...
  void _handleMessageSend() async {
    ...
    try {
      // 收集用户消息数据
      final multimodalService = ref.read(multimodalDataServiceProvider);
      await multimodalService.collectTextData(_sessionId, message, label: 'user_message');
      ...
      if (hasImage) {
        ...
        // 收集AI回复
        await multimodalService.collectTextData(_sessionId, aiResponse, label: 'ai_response');
      }
  ...
  void _collectMultimodalData(String data, String type) async {
    try {
      ...
      switch (type) {
        case 'speech':
          await multimodalService.collectAudioData(_sessionId, data, label: 'user_speech');
          break;
        case 'text':
          await multimodalService.collectTextData(_sessionId, data, label: 'user_text');
          break;
        case 'environment':
          // 环境数据采集 - 将环境信息转换为文本格式
          final brightness = MediaQuery.of(context).platformBrightness.toString();
          final time = DateTime.now().hour;
          final environmentInfo = 'env:brightness=$brightness;time=$time;device=mobile';
        ...
        success = await speechService.startListening(
          onResult: (text) {
            if (text.isNotEmpty) {
              ...
              if (text.trim().length > 5 && (text.endsWith('。') || text.endsWith('？') || text.endsWith('！'))) {
                _handleMessageSend();
              }
            }
          },
    ...
    try {
      ...
      if (result != null && result is Map<String, dynamic>) {
        ...
        setState(() {
          ...
          _messages.add({
            'sender': 'ai',
            'content': '根据脉诊分析，您的脉象呈现${result['pulseType']}特征，${result['analysis']}。\n\n这可能与${result['relation']}有关，建议您${result['suggestion']}。',
            'timestamp': DateTime.now(),
            'isRead': true,
          });
        });
  ...
  void _startFaceDiagnosis() async {
    try {
      ...
      setState(() {
        ...
        _messages.add({
          'sender': 'ai',
          'content': '面诊分析功能正在开发中，将很快推出。面诊是中医传统诊断方法，通过观察面部色泽、五官特征等，判断内脏功能状态。\n\n传统中医认为，面部不同区域对应不同的脏腑:\n· 额头 - 对应心脏\n· 鼻子 - 对应脾胃\n· 双颊 - 对应肺部\n· 下巴 - 对应肾脏\n\n请持续关注，面诊功能即将上线。',
          'timestamp': DateTime.now(),
          'isRead': true,
        });
      });
  ...
  void _startConstitutionAssessment() async {
    try {
      ...
      setState(() {
        ...
        _messages.add({
          'sender': 'ai',
          'content': '体质测评功能正在开发中，将很快推出。中医体质理论将人的体质分为九种基本类型：平和质、气虚质、阳虚质、阴虚质、痰湿质、湿热质、气郁质、血瘀质和特禀质。\n\n不同体质的人在生理、心理特点以及对环境的适应能力等方面都有所不同，也更易患某些疾病。体质测评将帮助您了解自己的体质类型，从而提供针对性的养生建议。\n\n请持续关注，体质测评功能即将上线。',
          'timestamp': DateTime.now(),
          'isRead': true,
        });
      });
  ...
  void _startMBTITest() async {
    try {
      ...
      setState(() {
        _messages.add({
          'sender': 'ai',
          'content': 'MBTI(迈尔斯-布里格斯类型指标)是一种流行的人格类型测试，可以帮助您了解自己的性格特点和沟通方式。准备好开始测试了吗？',
          'timestamp': DateTime.now(),
          'isRead': true,
        });
      });
      ...
      setState(() {
        ...
        _messages.add({
          'sender': 'ai',
          'content': 'MBTI测试功能正在开发中，将很快推出。MBTI将人格分为16种类型，基于四个维度的偏好：\n\n· 精力方向：外向(E)或内向(I)\n· 信息获取：感觉(S)或直觉(N)\n· 决策方式：思考(T)或情感(F)\n· 生活方式：判断(J)或认知(P)\n\n了解您的MBTI类型有助于更好地认识自己的优势和挑战，改善人际关系，指导职业发展。\n\n请持续关注，MBTI测试功能即将上线。',
          'timestamp': DateTime.now(),
          'isRead': true,
        });
      });
  ...
  void _startEnneagramTest() async {
    try {
      ...
      setState(() {
        ...
        _messages.add({
          'sender': 'ai',
          'content': '九型人格测试功能正在开发中，将很快推出。九型人格将人分为九种基本类型：\n\n· 一号：完美主义者\n· 二号：助人者\n· 三号：成就者\n· 四号：个人主义者\n· 五号：思想家\n· 六号：忠诚者\n· 七号：探险家\n· 八号：领导者\n· 九号：调解者\n\n了解您的九型人格类型有助于认识自己的核心动机、恐惧和防御机制，促进个人成长和自我发展。\n\n请持续关注，九型人格测试功能即将上线。',
          'timestamp': DateTime.now(),
          'isRead': true,
        });
      });
  ...
  void _startEmotionAnalysis() async {
    try {
      ...
      setState(() {
        ...
        _messages.add({
          'sender': 'ai',
          'content': '情绪分析功能正在开发中，将很快推出。情绪分析通过分析表情、语音、文字等多种方式，评估您的情绪状态，包括喜悦、悲伤、愤怒、恐惧、厌恶、惊讶等基本情绪，以及更复杂的情绪组合。\n\n中医认为情绪与五脏关系密切：\n· 怒伤肝\n· 喜伤心\n· 思伤脾\n· 忧伤肺\n· 恐伤肾\n\n了解并管理情绪对身心健康至关重要。\n\n请持续关注，情绪分析功能即将上线。',
          'timestamp': DateTime.now(),
          'isRead': true,
        });
      });
  ...
  void _startStressAssessment() async {
    try {
      ...
      setState(() {
        ...
        _messages.add({
          'sender': 'ai',
          'content': '压力评估功能正在开发中，将很快推出。压力评估通过问卷、心率变异性(HRV)分析、皮肤电反应等方式，综合评估您的压力水平。\n\n长期的高压力状态可能导致：\n· 睡眠质量下降\n· 免疫功能减弱\n· 消化问题\n· 心血管健康风险增加\n· 情绪波动\n\n中医认为过度的压力会导致气机郁滞，影响气血运行。了解自己的压力状态，采取适当的减压方法，对维护健康至关重要。\n\n请持续关注，压力评估功能即将上线。',
          'timestamp': DateTime.now(),
          'isRead': true,
        });
      });
      ...
      if (imagePath != null) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            ...
            action: contentType == 'tongue' ? SnackBarAction(
              label: '舌诊分析',
              onPressed: () {
                _handleTongueImageDetected(imagePath);
              },
  ...
  Future<void> _handleTongueImageDetected(String imagePath) async {
    ...
    // 调用舌诊服务
    try {
      final result = await context.router.push(TongueDiagnosisRoute(imagePath: imagePath));
  ...
  Widget _buildCallButton() {
    ...
    // 通话活跃时为红色，否则为绿色
    final buttonColor = _isCallActive 
        ? Colors.red 
